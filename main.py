...
async def get_current_user(request: Request):
    ...
    try:
        decoded_token = firebase_auth.verify_id_token(id_token.replace("Bearer ", ""))
...
async def verify_role(request: Request, required_roles: list = None):
    ...
    # ✅ Fetch roles list instead of single role
...
_NOTIFICATION_TTL_HOURS = 24
    ...
    async def get_user_roles(request: Request):
        uid, user_roles = await verify_role(request)
