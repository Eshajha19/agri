...
# ---------------------------------------------------------------------------
# Structured error codes — stable machine-readable identifiers that allow
# verify_role() to map reasons deterministically without string matching.
# ---------------------------------------------------------------------------

class RBACError(Exception):
    """Base RBAC error with a stable machine-readable code."""
...
class MissingTokenError(RBACError):
    def __init__(self):
        super().__init__(
...
class InvalidTokenError(RBACError):
    def __init__(self, detail: str = "Invalid or expired authorization token"):
        super().__init__("invalid_authentication_token", detail, 401)


class RevokedTokenError(RBACError):
    def __init__(self):
        super().__init__(
...
class UserProfileNotFoundError(RBACError):
    def __init__(self):
        super().__init__("user_profile_not_found", "User profile not found", 403)


class StaleTokenError(RBACError):
    def __init__(self):
        super().__init__("stale_authentication_token", STALE_TOKEN_DETAIL, 403)


class InvalidProfileRoleError(RBACError):
    def __init__(self):
        super().__init__(
            "invalid_profile_role",
            "Invalid role assigned to user profile",
            403,
        )


class AuthServiceUnavailableError(RBACError):
    def __init__(self):
        super().__init__(
            "authorization_service_unavailable",
            "Authentication database service temporarily unavailable",
            503,
        )
...
class RBACMatrix:
    ROLE_PERMISSIONS: Dict[Role, List[Permission]] = {
        Role.ADMIN: [
            Permission.FINANCE_CREATE, Permission.FINANCE_READ_ALL,
            Permission.FINANCE_UPDATE_ALL, Permission.FINANCE_DELETE,
            Permission.SUPPLY_CHAIN_CREATE, Permission.SUPPLY_CHAIN_READ,
            Permission.SUPPLY_CHAIN_UPDATE, Permission.SUPPLY_CHAIN_DELETE,
            Permission.NOTIFICATIONS_READ, Permission.NOTIFICATIONS_CREATE,
            Permission.NOTIFICATIONS_DELETE, Permission.REPORTS_READ_ALL,
            Permission.REPORTS_DELETE, Permission.QUALITY_ASSESS,
            Permission.QUALITY_READ, Permission.SEEDS_VERIFY, Permission.SEEDS_READ,
            Permission.WHATSAPP_SUBSCRIBE, Permission.WHATSAPP_TRIGGER,
            Permission.WHATSAPP_WEBHOOK, Permission.SYSTEM_LOG,
            Permission.SYSTEM_ADMIN, Permission.RAG_QUERY,
            Permission.CLIMATE_SIMULATE, Permission.REPORTS_CREATE,
            Permission.FINANCE_UPDATE_OWN, Permission.FINANCE_READ_OWN,
        ],
        Role.EXPERT: [
            Permission.FINANCE_READ_ALL, Permission.SUPPLY_CHAIN_READ,
            Permission.NOTIFICATIONS_READ, Permission.REPORTS_READ_ALL,
            Permission.REPORTS_CREATE, Permission.QUALITY_ASSESS,
            Permission.QUALITY_READ, Permission.SEEDS_VERIFY, Permission.SEEDS_READ,
            Permission.RAG_QUERY, Permission.CLIMATE_SIMULATE,
        ],
        Role.FARMER: [
            Permission.FINANCE_CREATE, Permission.FINANCE_READ_OWN,
            Permission.FINANCE_UPDATE_OWN, Permission.SUPPLY_CHAIN_CREATE,
            Permission.SUPPLY_CHAIN_READ, Permission.SUPPLY_CHAIN_UPDATE,
            Permission.NOTIFICATIONS_READ, Permission.REPORTS_CREATE,
            Permission.REPORTS_READ_OWN, Permission.QUALITY_ASSESS,
            Permission.QUALITY_READ, Permission.SEEDS_READ,
            Permission.WHATSAPP_SUBSCRIBE, Permission.RAG_QUERY,
            Permission.CLIMATE_SIMULATE,
        ],
        Role.VENDOR: [
            Permission.SUPPLY_CHAIN_READ, Permission.SUPPLY_CHAIN_CREATE,
            Permission.SUPPLY_CHAIN_UPDATE, Permission.NOTIFICATIONS_READ,
            Permission.QUALITY_READ, Permission.SEEDS_READ,
            Permission.WHATSAPP_SUBSCRIBE, Permission.RAG_QUERY,
            Permission.CLIMATE_SIMULATE,
        ],
        Role.SYSTEM: [
            Permission.FINANCE_CREATE, Permission.FINANCE_READ_ALL,
            Permission.FINANCE_UPDATE_ALL, Permission.SUPPLY_CHAIN_CREATE,
            Permission.SUPPLY_CHAIN_READ, Permission.SUPPLY_CHAIN_UPDATE,
            Permission.NOTIFICATIONS_CREATE, Permission.SYSTEM_ADMIN,
            Permission.SYSTEM_LOG, Permission.WHATSAPP_WEBHOOK,
        ],
        Role.GUEST: [
            Permission.RAG_QUERY, Permission.CLIMATE_SIMULATE, Permission.SEEDS_READ,
        ],
    }
...
class RBACManager:
    ...
    async def get_user_role(request: Request) -> Role:
        try:
            ctx = await RBACManager.resolve_auth_context(
                request, allow_unauthenticated=True,
            )
        except RBACError:
            return Role.GUEST
        if ctx is None:
    ...
    async def verify_permission(
        ...
        ctx = await RBACManager.resolve_auth_context(
            request, allow_unauthenticated=False,
        )
    ...
    async def raise_if_unauthorized(
        ...
        if not has_permission:
            try:
                ...
                role_label = ctx.role
            except (HTTPException, RBACError):
                role_label = "unknown"
            logger.warning(
                "Unauthorized access attempt with role: %s, required: %s",
                role_label, [p.value for p in required_permissions],
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=detail,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, request: Request = None, **kwargs):
            ...
            await RBACManager.raise_if_unauthorized(
                request, required_perms, require_all=require_all,
...
def print_rbac_matrix() -> str:
    lines = ["\n" + "=" * 100, "RBAC ENFORCEMENT MATRIX", "=" * 100]
    for role in Role:
