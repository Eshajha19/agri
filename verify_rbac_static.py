...
print("=" * 80)
print("RBAC ENFORCEMENT VERIFICATION (Static Analysis)")
print("=" * 80)
...
try:
    ...
    print("✓ RBAC matrix has all role definitions")
...
try:
    ...
    print("✓ main.py has all required RBAC imports")
    
except Exception as e:
    print(f"✗ main.py import verification failed: {e}")
    exit(1)
...
try:
    ...
    print("✓ RBAC middleware is registered in main.py")
    
except Exception as e:
    print(f"✗ Middleware registration verification failed: {e}")
    exit(1)
        ...
        if route in main_content:
            ...
            if route_match != -1:
                ...
                if permission in route_section or 'RBACManager.raise_if_unauthorized' in route_section:
                    print(f"  ✓ {route:35} protected with {permission}")
                    protected_count += 1
                else:
                    print(f"  ⚠ {route:35} - needs review")
        else:
            print(f"  ? {route:35} - not found in main.py")
...
try:
    ...
    └── Can view FAQ
    """)
    print("✓ RBAC role hierarchy defined")
    
except Exception as e:
    print(f"✗ Role hierarchy display failed: {e}")
    exit(1)
...
try:
    ...
    print(f"✗ Endpoint summary failed: {e}")
    exit(1)
...
try:
    ...
    print(f"✗ Security features checklist failed: {e}")
    exit(1)
